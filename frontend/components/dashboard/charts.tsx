...
export function InventoryOverview({
  ...
  movementData,
  totalBalance,
}: Readonly<{ data: InventoryCategory[]; movementData: ProductionPoint[]; totalBalance: number }>) {
  const hasData = movementData.some((item) => Number(item.planned || 0) > 0 || Number(item.completed || 0) > 0);

  return (
    <DashboardCard className="overflow-hidden lg:col-span-5" delay={0.18}>
      <PanelTitle href="/inventory-logs" title="Inventory Overview" />
      {hasData ? (
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_190px]">
          <div className="h-52 min-w-0 sm:h-64">
            <ResponsiveContainer height="100%" width="100%">
              <AreaChart data={movementData}>
                <defs>
                  <linearGradient id="inventoryInGradient" x1="0" x2="0" y1="0" y2="1">
                    <stop offset="5%" stopColor="#19C93B" stopOpacity={0.24} />
                    <stop offset="95%" stopColor="#19C93B" stopOpacity={0.03} />
                  </linearGradient>
                  <linearGradient id="inventoryOutGradient" x1="0" x2="0" y1="0" y2="1">
                ...
                <Tooltip content={<ChartTooltip />} cursor={{ stroke: "#19C93B", strokeDasharray: "4 4" }} />
                <Area dataKey="planned" fill="url(#inventoryInGradient)" name="IN" stroke="#19C93B" strokeWidth={3} type="monotone" />
                <Area dataKey="completed" fill="url(#inventoryOutGradient)" name="OUT" stroke="#38BDF8" strokeWidth={3} type="monotone" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ...
        <EmptyChartState label="No inventory trend data yet" />
      )}
    </DashboardCard>
  );
}
...
export function ProductionOverview({ data }: Readonly<{ data: ProductionPoint[] }>) {
  const hasData = data.some((item) => Number(item.planned || 0) > 0 || Number(item.completed || 0) > 0);
    ...
    <DashboardCard className="lg:col-span-7" delay={0.22}>
      ...
      <div className="mt-4 h-52 sm:mt-5 sm:h-64">
        {hasData ? (
          ...
          <EmptyChartState label="No production trend data yet" />
        )}
      </div>
    </DashboardCard>
  );
...
function EmptyChartState({ label }: Readonly<{ label: string }>) {
  return (
    <div className="flex h-full min-h-44 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 text-center text-sm font-semibold text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
      {label}
    </div>
  );
}
